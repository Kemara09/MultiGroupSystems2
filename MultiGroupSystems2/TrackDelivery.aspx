...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <style>
        ...
        .search-box-wrapper { display: flex; gap: 10px; margin-bottom: 25px; }
        .search-input {
            flex: 1; padding: 12px 18px; border: 1.5px solid #e2e8f0;
            border-radius: 25px; font-size: 14px; outline: none; transition: border-color 0.2s;
            background: #ffffff; color: #0f172a;
        }
        .search-input:focus { border-color: #008296; }

        .btn-search {
            background: #008296; color: #ffffff; border: none; padding: 12px 24px;
            border-radius: 25px; font-weight: 600; font-size: 14px; cursor: pointer;
        }
        .btn-search:hover { background: #006878; }

        .card-header { text-align: center; margin-bottom: 20px; }
        .status-heading { font-size: 24px; font-weight: 700; color: #0f172a; margin: 0; }

        .stepper-wrapper { position: relative; margin: 25px 10px 30px 10px; }
        .progress-bg-bar {
            position: absolute; top: 12px; left: 8%; width: 84%; height: 8px;
            background-color: #e5e7eb; border-radius: 4px; z-index: 1;
        }
        .progress-fill-bar {
            position: absolute; top: 12px; left: 8%; width: 0%; height: 8px;
            background-color: #008296; border-radius: 4px; z-index: 2;
            transition: width 1.2s cubic-bezier(0.4, 0, 0.2, 1);
        }

        .steps-container { position: relative; z-index: 3; display: flex; justify-content: space-between; }
        ...
        .dot-circle {
            ...
            color: transparent; font-weight: bold; font-size: 14px;
            transition: background-color 0.4s ease, transform 0.3s ease;
        }
        .step-item.active .dot-circle,
        .step-item.completed .dot-circle { background-color: #008296; color: #ffffff; transform: scale(1.08); }
        .step-label { font-size: 13px; color: #6b7280; font-weight: 500; line-height: 1.2; }
        .step-item.active .step-label { color: #0f172a; font-weight: 700; }

        .action-grid { display: flex; gap: 15px; margin-top: 25px; }
        .action-btn {
            flex: 1; padding: 12px; border: 1px solid #e2e8f0; border-radius: 15px;
            background: #ffffff; font-weight: 600; color: #334155; cursor: pointer;
        }
        .action-btn:hover { background: #f8fafc; }

        #deliveryMap {
            width: 100%; height: 380px; border-radius: 16px; margin-top: 25px;
            border: 1px solid #e2e8f0; display: none;
        }

        .error-msg { color: #dc2626; text-align: center; font-weight: 600; margin-bottom: 15px; display: block; }
    </style>
    ...
    <div class="track-card-container">
        ...
        <div class="card-header">
            <h2 class="status-heading"><asp:Label ID="lblStatusHeading" runat="server" Text="Ordered"></asp:Label></h2>
        </div>
